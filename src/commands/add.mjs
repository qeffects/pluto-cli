...
import { addHelp, helpParser } from "../helpStrings.mjs";
...
export const addCommand = async () => {
    ...
    let argPckgName, argPckgCommit, argPckgTag, argPckgLatest;
    ...
    if (!moduleMap.availablePackages.includes(argPckgName)) {
        PlutoError(
            `Couldn't find package: ${chalk.red(
                argPckgName
            )}! \n Try running ${chalk.bold(
                COMMAND_FETCH
            )} to update your local depot\n Or ${chalk.bold(
                COMMAND_LIST
    ...
    const manifest = await ReadModuleManifest(
        moduleMap.packageToFolderMap[argPckgName]
    );
    if (argv.c) {
    ...
    if (argPckgCommit) {
        ...
        if (!commitExists) {
            PlutoError(
                `Couldn't find commit hash ${chalk.red(
                    argPckgCommit
                )} for ${chalk.green(argPckgName)}, try running ${chalk.bold(
                    COMMAND_FETCH
                )} to update the local depot or ${chalk.bold(
                    `${COMMAND_INSPECT} ${argPckgName} -c`
                )} to see the list of available commits`
            );

            process.exit();
        } else {
            PlutoSuccess(
                `Found commit ${chalk.red(argPckgCommit)} for ${chalk.green(
                    argPckgName
                )}!`
            );
        }
    ...
    if (argPckgTag) {
        ...
        if (!tagExists) {
            PlutoError(
                `Couldn't find tag ${chalk.red(argPckgTag)} for ${chalk.green(
                    argPckgName
                )}, try running ${chalk.bold(
                    COMMAND_FETCH
                )} to update the local depot or ${chalk.bold(
                    `${COMMAND_INSPECT} ${argPckgName} -t`
                )} to see the list of available tags`
            );
            process.exit();
        } else {
            PlutoSuccess(
                `Found tag ${chalk.red(argPckgTag)} for ${chalk.green(
                    argPckgName
                )}!`
            );
        }
    ...
    if (argv.l || (!argPckgCommit && !argPckgTag)) {
        ...
        PlutoLog(
            ...
            `${chalk.green(argPckgName)}@${chalk.red("latest")}`
        );
    } else if (argPckgCommit || argPckgTag) {
        PlutoLog(
            `Adding direct dependency`,
            `${chalk.green(argPckgName)}${chalk.red(
                argPckgCommit ? "#" + argPckgCommit : "@" + argPckgTag
            )}`
        );
    }
    ...
    const areFrameworksCompatible = checkFrameworks(
        plutoModule.frameworks,
        manifest.frameworks
    );
    const areRuntimesCompatible = checkRuntime(
        plutoModule.runtime,
        manifest.runtime
    );
    ...
    if (!areFrameworksCompatible) {
        PlutoError(
            INCOMPATIBLE_FRAMEWORK(manifest.frameworks, plutoModule.frameworks)
        );
    } else {
        PlutoSuccess(
            COMPATIBLE_FRAMEWORK(manifest.frameworks, plutoModule.frameworks)
        );
    }
    ...
    if (hasIncompatibility && !argv.b) {
        const add = (
            await rl.question(`Compatibility problem found, add anyways?`)
        ).trim();
