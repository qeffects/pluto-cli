...
import { COMMAND_FETCH, COMMAND_UPDATE, LOCK_FILE, MODULE_DIRECTORY, MODULE_MANIFEST_FILENAME } from "../constants.mjs";
...
if (argCommit && isCommitFound) {
    ...
    PlutoHelp(`Remember that you have to update your requires now, the package must imported like require(${chalk.green(`"${argName}#${argCommit}"`)})`);
} else if (argCommit!==undefined) {
    PlutoError(
        `Couldn't find ${argCommit} in commit history, exiting...`
    );
    process.exit(1);
}
...
if (argTag && modManifest.tags.includes(argTag)) {
    ...
    PlutoHelp(`Remember that you have to update your requires now, the package must imported like require(${chalk.green(`"${argName}@${argTag}"`)})`);
} else if (argTag!==undefined) {
    PlutoError(
        `Couldn't find ${argTag} in tags, exiting...`
    );
    process.exit(1);
}
...
if (!argTag&&!argCommit) {
    ...
    delete plutoModule.dependencies[argName].gitTag;
    delete plutoModule.dependencies[argName].gitCommit;
    PlutoHelp(`Remember that you have to update your requires now, the package must imported like require(${chalk.green(`"${argName}"`)})`);
}
...
PlutoHelp(`For the changes to take place you have to run ${chalk.blue.bold(COMMAND_UPDATE)} !`);