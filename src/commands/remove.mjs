...
import { helpParser, removeHelp } from "../helpStrings.mjs";

// remove name

export const removeCommand = async () => {
    const argv = minimist(process.argv.slice(3));
    if (helpParser(argv, removeHelp)){
        return;
    };
    PlutoHello("remove");
    ...
    const moduleShorthand = generateModuleShorthand(
        plutoModule.dependencies[argName],
        argName
    );
    ...
    while (packagesToRemove.length > 0) {
        ...
        for (let index = 0; index < packagesToRemove.length; index++) {
            ...
            plutoLock.flatPackageList = plutoLock.flatPackageList.filter(
                (v) => {
                    return v !== packageName;
                }
            );

            plutoLock.realPackageList = plutoLock.realPackageList.filter(
                (v) => {
        ...
        PlutoLog(
            "L",
            `Found more modules to remove: ${chalk.blue(
    ...
    for (let index = 0; index < removedFolders.length; index++) {
        ...
        await fs.rm(`${cwd}/${MODULE_DIRECTORY}/${folderName}`, {
            recursive: true,
            force: true,
        });
        PlutoLog(
            "L",
            `Removed folder ${`${cwd}/${MODULE_DIRECTORY}/${chalk.green(
                folderName
            )}`}`
        );
    }
    ...
    await fs.writeFile(
        cwd + "/" + LOCK_FILE,
        JSON.stringify(plutoLock, null, 4)
    );
